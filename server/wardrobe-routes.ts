...
type Ctx = {
  ...
  userFromQuery: () => AsyncValue<{ id: string } | null>;
};
...
export async function handleWardrobeRoutes(ctx: Ctx): Promise<boolean> {
  ...
  if (method === "POST" && path === "/api/wardrobe/recognize") {
    ...
    const saved = await wardrobe.add(user.id, {
      title: item.title,
  ...
  if (method === "DELETE" && path.startsWith("/api/wardrobe/items/")) {
    ...
    if (!(await wardrobe.remove(user.id, id))) {
  ...
  if (method === "PATCH" && path.startsWith("/api/wardrobe/items/")) {
    ...
    const updated = await wardrobe.update(user.id, id, patch);
    if (!updated) {
      json(res, 404, { error: "单品不存在。" });
      return true;
    }
  ...
  if (method === "GET" && path.startsWith("/api/wardrobe/photo/")) {
    ...
    const user = (await userFromHeader()) ?? (await ctx.userFromQuery());
    if (!user) {
      json(res, 401, { error: "Not signed in." });
      return true;
    }
    const id = decodeURIComponent(path.split("/").pop() ?? "");
    if (wardrobe.photo) {
      const photo = await wardrobe.photo(user.id, id);
      if (!photo) {
    ...
    const file = await wardrobe.photoPath?.(user.id, id);
    if (!file) {
      json(res, 404, { error: "照片不存在。" });
      return true;
    }
